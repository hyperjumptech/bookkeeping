...
func (repo *MySqlDBRepository) ClearTables(ctx context.Context) error {
	...
	for _, t := range tablesToDrop {
		_, err := repo.db.ExecContext(ctx, fmt.Sprintf("DROP TABLE %s", t))
		if err != nil {
...
func (repo *MySqlDBRepository) IsConnected() bool {
	if repo.db == nil || repo.connected == false {
		return false
	}
	return true
}
...
func (repo *MySqlDBRepository) InsertAccount(ctx context.Context, rec *AccountRecord) (string, error) {
	...
	if len(rec.CurrencyCode) > 3 {
		llog.Errorf("Currency code %s is too long. Should not more than 3 digit", rec.CurrencyCode)
	...
	theUser, ok := ctx.Value(UserIDContextKey).(string)
	if !ok {
		llog.Errorf("UserContext Key %s is not in context",UserIDContextKey)
...
func (repo *MySqlDBRepository) ListAccountByCoa(ctx context.Context, coa string, sort string, offset, length int) ([]*AccountRecord, error) {
	...
	if err != nil {
		llog.Errorf("error while lisitng account by coa. got %s", err.Error())
		return nil, err
	}
	defer rows.Close()
	ret := make([]*AccountRecord, 0)
...
func (repo *MySqlDBRepository) InsertJournal(ctx context.Context, rec *JournalRecord) (string, error) {
	...
	q := "INSERT INTO journals(" +
		"journal_id, journaling_time, description, is_reversal, reversed_jounal_id, total_amount, created_at, created_by, is_deleted" +
		") VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, false)"
	args := []interface{} {
		rec.JournalID, rec.JournalingTime, rec.Description, rec.Description, rec.IsReversal, rec.ReversedJournalId, rec.TotalAmount, rec.CreatedAt, rec.CreatedBy,
	}
	_, err := repo.db.ExecContext(ctx, q, args...)
	if err != nil {
...
func (repo *MySqlDBRepository) UpdateJournal(ctx context.Context, rec *JournalRecord) error {
	...
	rec.CreatedAt = time.Now()
	q := "UPDATE journals " +
		"set journaling_time=?, description=?, is_reversal=?, reversed_jounal_id=?, total_amount=?, created_at=?, created_by=?" +
		" WHERE journal_id=? AND is_deleted=false"
	args := []interface{} {
		rec.JournalingTime, rec.Description, rec.Description, rec.IsReversal, rec.ReversedJournalId, rec.TotalAmount, rec.CreatedAt, rec.CreatedBy, rec.JournalID,
	}
	_, err := repo.db.ExecContext(ctx, q, args...)
	if err != nil {
...
func (repo *MySqlDBRepository) ListJournal(ctx context.Context, sort string, offset, length int) ([]*JournalRecord, error) {
	...
	q := "SELECT journal_id, journaling_time, description, is_reversal, reversed_jounal_id, total_amount, created_at, created_by" +
...
func (repo *MySqlDBRepository) GetJournalByReversalID(ctx context.Context, journalID string) (*JournalRecord, error){
	...
	q := "SELECT  journal_id, journaling_time, description, is_reversal, reversed_jounal_id, total_amount, created_at, created_by" +
		" FROM journals WHERE reversed_jounal_id=? AND is_deleted=false"
	row := repo.db.QueryRowxContext(ctx, q, journalID)
	if row.Err() != nil {
...
func (repo *MySqlDBRepository) CountJournalByTimeRange(ctx context.Context, timeFrom, timeTo time.Time) (int,error) {
	...
	err := row.Scan(&count)
	if err != nil {
		llog.Errorf("error while scanning journals count whend finding journal by time range. got %s", err.Error())
		return 0, err
	}
	return count, nil
}
...
func (repo *MySqlDBRepository) InsertCurrency(ctx context.Context, rec *CurrenciesRecord) (string, error) {
	...
	if len(rec.Code) > 3 {
		llog.Errorf("Currency code %s is too long. Should not more than 3 digit", rec.Code)
		return "", errors.ErrStringDataTooLong
	}
	if len(rec.Name) > 10 {
		llog.Errorf("Currency name %s is too long. Should not more than 10 digit", rec.Code)
		return "", errors.ErrStringDataTooLong
	}
	if len(rec.CreatedBy) > 16 {
		rec.CreatedBy = rec.CreatedBy[:16]
